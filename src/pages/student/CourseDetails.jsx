  ...
  return courseData ?  (
    ...
    <div className='flex md:flex-row flex-col-reverse gap-10 relative items-start justify-between md:px-36 px-8 pt-20 text-left'>
      ...
      <div className='max-w-xl z-10 text-gray-500'>
        ...
        {/* review and ratings */}
<div className="flex items-center mt-2  space-x-2 pt-3 text-sm">
          <p className="text-yellow-600 font-medium">{calculateRating(courseData)}</p>
          <div className="flex">
            {[...Array(5)].map((_, i) => (
              <img 
                key={i} 
                src={Math.floor(calculateRating(courseData)) >= i + 1 ? assets.star : assets.star_blank } 
                alt="star" 
                className="w-4 h-4" 
                onError={(e) => {
          ...
          <p className="text-gray-600 text-sm">({courseData.courseRatings.length} {courseData.courseRatings.length > 1 ? 'ratings' : 'rating'} )</p>
  ...
  {courseData.courseContent.map((chapter , index)=> (
    <div key= {index} className='border border-gray-300 bg-white mb-2 rounded'>
      ...
      <div className={`overflow-hidden transition-all duration-300 ${openSection[index] ? 'max-h-96' : 'max-h-0'}`}>
        <ul className='list-disc md:pl-10pr-4 py-2 text-gray-600 border-t border-gray-300'>
      ...
      :<img src={courseData.courseThumbnail} alt={courseData.courseTitle || 'Course Thumbnail'} className="w-full h-auto" />
    }
  <div className='p-5'>
    <div className='flex items-center gap-2'>
